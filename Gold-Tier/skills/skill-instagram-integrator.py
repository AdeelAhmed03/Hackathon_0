#!/usr/bin/env python3
"""
Instagram Integrator Skill - Gold Tier

Handles Instagram integrations: posts messages and generates summaries.
Follows Gold Spec with Instagram-specific adaptations.

Features:
- Reads Company_Handbook.md for tone guidelines
- Creates Instagram posts in data/Plans/IG_POST_{id}.md format
- Requires HITL: Create approval request for /Pending_Approval/IG_POST_{id}.md
- On approval: Invokes social-mcp-ig with {"action": "post", "message": text}
- For summary: Polls via instagram_watcher.py or MCP; generates SUMMARY_IG_{date}.md with engagement
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - InstagramIntegrator - %(levelname)s - %(message)s",
    handlers=[
        logging.FileHandler(LOGS_DIR / "instagram_integrator.log"),
        logging.StreamHandler(),
    ],
)
logger = logging.getLogger("InstagramIntegrator")
...
def log_action(action_type: str, details: dict, result: str = "success"):
    ...
    try:
        ...
        correlation_id = f"ig_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        audit_log_action(
            action_type=action_type,
            actor="instagram_integrator",
            target="social-mcp-ig",
            parameters=details,
            result=result,
...
def create_approval_request(plan_file: Path, correlation_id: str) -> bool:
    ...
    try:
        ...
        message_line = next((line for line in lines if not line.startswith('---') and line.strip()), "Instagram post")

        # Create approval request
        approval_file = PENDING_APPROVAL_DIR / f"IG_POST_{correlation_id}.md"
        approval_content = f"""---
...
Please review and approve this Instagram post before it goes live.
"""
...
def generate_instagram_summary(engagement_data: dict, date_str: str) -> Path:
    """
    Generate an Instagram engagement summary in SUMMARY_IG_{date}.md format.
    ...
    """
    summary_file = BRIEFINGS_DIR / f"SUMMARY_IG_{date_str}.md"
...
## Additional Insights
This summary was automatically generated by the Instagram Integrator skill.
"""

    summary_file.write_text(summary_content, encoding="utf-8")
    logger.info(f"Instagram summary created: {summary_file.name}")
...
# ── MAIN INSTAGRAM INTEGRATOR SKILL ───────────────────────────────────────
class InstagramIntegrator:
    """Main Instagram integrator skill class."""

    def __init__(self):
        self.handbook_tone = read_company_handbook_tone()
        logger.info("Instagram Integrator initialized")
    ...
    def post_message(self, message: str = None, plan_content: str = None) -> dict:
        """
        Post a message to Instagram following the Gold Spec pattern.
        ...
        try:
            ...
            if plan_content:
                ...
                plan_file = PLANS_DIR / f"IG_POST_{correlation_id}.md"
...
"""
            ...
            logger.info(f"Instagram post plan created: {plan_file.name}")
            ...
            result = {
                ...
                "plan_file": str(plan_file.name),
                "approval_file": f"IG_POST_{correlation_id}.md",
            ...
            logger.error(f"Error in Instagram post process: {e}")
            log_action("post_plan_created", {
                "error": str(e),
            ...
            return {
                "success": False,
                "error": str(e),
                "message": f"Failed to create Instagram post: {e}"
            }

    def handle_approved_post(self, approved_file_path: Path) -> dict:
        """
        Handle an Instagram post that has been approved by HITL.
        ...
        """
        logger.info(f"Handling approved Instagram post: {approved_file_path.name}")
        ...
        try:
            ...
            match = re.search(r'IG_POST_([a-zA-Z0-9_]+)', approved_file_path.name)
            if not match:
            ...
            plan_candidates = list(PLANS_DIR.glob(f"IG_POST_{correlation_id}.*"))
            plan_file = None
            if plan_candidates:
...
"""
            ...
            # Step 3: Invoke social-mcp-ig with the message
            mcp_params = {
                "action": "post",
                "message": message,
                "account_id": os.environ.get("IG_ACCOUNT_ID", "default_account"),  # Use env var if available
                "correlation_id": correlation_id
            }
    ...
    def generate_summary(self, engagement_data: dict = None) -> Path:
        """
        Generate an Instagram engagement summary.
        ...
        Returns:
            Path: Path to the generated summary file
        """
        logger.info("Generating Instagram engagement summary")
        ...
        summary_file = generate_instagram_summary(engagement_data, date_str)

        logger.info(f"Instagram summary generated: {summary_file.name}")
        return summary_file

    def fetch_engagement_data(self) -> dict:
        """
        Fetch engagement data from Instagram using the watcher or MCP.

        Returns:
            dict: Engagement metrics
        """
        logger.info("Fetching Instagram engagement data")

        try:
            # Try to use the instagram watcher if available
            watcher_path = VAULT_DIR / "watcher" / "instagram_watcher.py"
            if watcher_path.exists():
                ...
                logger.info("Instagram watcher found, simulating engagement data fetch")
                return {
                    "likes": 120,
                    "shares": 25,
                    "comments": 15,
                ...
                logger.warning("Instagram watcher not found, using sample data")
                return {
                    "likes": 100,
                    "shares": 20,
                    "comments": 12,
...
# ── MAIN EXECUTION ────────────────────────────────────────────────────────
def main():
    """Main function to demonstrate the Instagram Integrator skill."""
    import argparse

    parser = argparse.ArgumentParser(description="Instagram Integrator Skill")
    parser.add_argument("--post", help="Message to post to Instagram")
    parser.add_argument("--approve", help="Process an approved Instagram post file")
    parser.add_argument("--summary", action="store_true", help="Generate Instagram engagement summary")
    ...
    integrator = InstagramIntegrator()
    ...
    if args.post:
        ...
        print("Running Instagram Integrator demo...")

        # Step 1: Create a post
        test_message = "Visual update: New product launch! Check out our latest innovation! #AIEmployee #VisualContent"
        print(f"\n1. Creating Instagram post: {test_message}")
        ...
        print("Instagram Integrator Skill")
        print("Usage:")
        print("  --post \"message\"          : Create an Instagram post (requires approval)")
        print("  --approve FILE            : Process an approved Instagram post")

...
Gold additions:
  - Weekly audit job (Sunday at CEO_BRIEF_HOUR) via audit_logic.py
  - CEO Briefing generation after audit completes
  - Force-trigger support for weekly_audit and ceo_briefing
...
AUDIT_SCHEDULE_TIME = f"{CEO_BRIEF_HOUR:02d}:00"
...
def job_weekly_audit(state, dry_run=False):
    """Run weekly audit via audit_logic.py and generate report in /Briefings/."""
    ...
    if dry_run:
        logger.info("[DRY RUN] Would run weekly audit")
        # Still create the task file so agent picks it up
        success = create_task_file(
            task_type=task_type,
            description="Weekly Audit",
            details=(
                "**Action Required:** Run weekly business audit.\n\n"
                "**Scope:** Vault metrics, accounting, social media, system health\n"
                "**Source:** audit_logic.py — run_weekly_audit()\n\n"
                "Use skill-weekly-audit to process this task.\n"
                "After audit completes, trigger skill-ceo-briefing."
            ),
            dry_run=dry_run,
        )
    else:
        # Run audit_logic directly
        try:
            sys.path.insert(0, str(VAULT_DIR))
            from audit_logic import run_weekly_audit, generate_audit_report
            audit_data = run_weekly_audit()
            audit_path = generate_audit_report(audit_data)
            logger.info(f"Weekly audit report generated: {audit_path}")
            success = True
        except Exception as e:
            logger.error(f"Audit failed, falling back to task file: {e}")
            success = create_task_file(
                task_type=task_type,
                description="Weekly Audit (auto-run failed)",
                details=(
                    "**Action Required:** Run weekly business audit manually.\n\n"
                    f"**Error:** {e}\n\n"
                    "Use skill-weekly-audit to process this task.\n"
                    "Run: python audit_logic.py --simulate"
                ),
                dry_run=False,
            )
...
def job_ceo_briefing(state, dry_run=False):
    """Generate CEO Briefing from latest audit data."""
    ...
    if dry_run:
        ...
        success = create_task_file(
            task_type=task_type,
            description="CEO Monday Briefing",
            details=(
                "**Action Required:** Generate Monday Morning CEO Briefing.\n\n"
                "**Source:** Latest AUDIT_{date}.md in /Briefings/\n"
                "**Template:** Executive Summary, Revenue, Bottlenecks, Suggestions\n\n"
                "Use skill-ceo-briefing to process this task."
            ),
            dry_run=dry_run,
        )
    else:
        try:
            ...
            audit_data = run_weekly_audit()
            briefing_path = generate_ceo_briefing(audit_data)
            logger.info(f"CEO Briefing generated: {briefing_path}")
            success = True
        except Exception as e:
            logger.error(f"CEO Briefing failed, falling back to task file: {e}")
            success = create_task_file(
                task_type=task_type,
                description="CEO Briefing (auto-run failed)",
                details=(
                    "**Action Required:** Generate CEO Briefing manually.\n\n"
                    f"**Error:** {e}\n\n"
                    "Use skill-ceo-briefing to process this task.\n"
                    "Run: python audit_logic.py --simulate"
                ),
...
def force_trigger(task_type, state, dry_run=False):
    ...
    if task_type == "linkedin_draft":
        ...
        job_weekly_audit(state, dry_run=dry_run)
    elif task_type == "ceo_briefing":
        job_ceo_briefing(state, dry_run=dry_run)
    else:
        logger.error(f"Unknown task type for force-trigger: {task_type}")
        logger.info("Available task types: linkedin_draft, weekly_audit, ceo_briefing")


# ── SCHEDULE CONFIGURATION ────────────────────────────────────────────────
def configure_schedules(state, dry_run=False):
    """Set up all scheduled jobs using the schedule library."""
    # Silver: Daily LinkedIn draft
    ...
    # Gold: Weekly audit on CEO_BRIEF_DAY
    day_map = {
    ...
    audit_day = day_map.get(CEO_BRIEF_DAY, schedule.every().sunday)
    logger.info(f"Configuring weekly audit: {CEO_BRIEF_DAY} at {AUDIT_SCHEDULE_TIME}")
    audit_day.at(AUDIT_SCHEDULE_TIME).do(
        job_weekly_audit, state=state, dry_run=dry_run
    )

    # Gold: CEO Briefing 1 hour after audit
    briefing_time = f"{CEO_BRIEF_HOUR + 1:02d}:00"
    logger.info(f"Configuring CEO briefing: {CEO_BRIEF_DAY} at {briefing_time}")
    audit_day_briefing = day_map.get(CEO_BRIEF_DAY, schedule.every().sunday)
    audit_day_briefing.at(briefing_time).do(
        job_ceo_briefing, state=state, dry_run=dry_run
    )
...
def main():
    ...
    if args.list_schedules:
        ...
        print(f"  - Weekly Audit:    {CEO_BRIEF_DAY} at {AUDIT_SCHEDULE_TIME}")
        print(f"  - CEO Briefing:    {CEO_BRIEF_DAY} at {CEO_BRIEF_HOUR + 1:02d}:00")
        return 0
    ...
    logger.info(f"Weekly audit schedule: {CEO_BRIEF_DAY} at {AUDIT_SCHEDULE_TIME}")
    logger.info(f"CEO briefing schedule: {CEO_BRIEF_DAY} at {CEO_BRIEF_HOUR + 1:02d}:00")

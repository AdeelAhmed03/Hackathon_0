...
Platinum enhancements:
  - Zone enforcement: VAULT_ENVIRONMENT=cloud restricts to draft-only operations
  - create_invoice_draft: Cloud-only tool — creates draft JSON in Plans/cloud/
  - approve_invoice: Local-only tool — approves a draft and posts to Odoo
  - post_invoice: Local-only tool — posts draft invoice to Odoo (action_post)
  - get_pending_drafts: Lists draft invoices awaiting local approval
...
# ── PLATINUM: Zone Enforcement ────────────────────────────────────────────
# VAULT_ENVIRONMENT: "cloud" = draft-only (no posting/payment execution)
#                    "local" = full access (approve, post, execute)
VAULT_ENVIRONMENT = os.environ.get("VAULT_ENVIRONMENT", "local")
IS_CLOUD = VAULT_ENVIRONMENT == "cloud"

# Platinum directories for draft/approval workflow
PLANS_CLOUD_DIR = VAULT_DIR_ROOT / "data" / "Plans" / "cloud"
PENDING_APPROVAL_DIR = VAULT_DIR_ROOT / "data" / "Pending_Approval" / "local"
APPROVED_DIR = VAULT_DIR_ROOT / "data" / "Approved"
UPDATES_DIR = VAULT_DIR_ROOT / "data" / "Updates"
...
# ── PLATINUM: Draft/Approval Workflow Tools ──────────────────────────────

def _require_zone(zone):
    """Raise if current VAULT_ENVIRONMENT doesn't match the required zone."""
    if zone == "cloud" and not IS_CLOUD:
        return  # cloud tools also work locally for testing
    if zone == "local" and IS_CLOUD:
        raise PermissionError(
            f"Tool restricted to local zone. Current environment: {VAULT_ENVIRONMENT}. "
            "Cloud agents must NOT execute posts/payments — use draft tools instead."
        )


def create_invoice_draft(partner_name, lines, currency="USD", notes=""):
    """Platinum: Create a DRAFT invoice file in Plans/cloud/ (no Odoo write).

    Cloud agent calls this to propose an invoice. The draft is a JSON file
    that the local agent can later approve and post to Odoo.
    ...
    total = sum(l.get("quantity", 1) * l.get("price_unit", 0) for l in lines)
    now = datetime.now()
    draft_id = f"odoo_inv_{now.strftime('%Y%m%d_%H%M%S')}"
    ...
    draft_data = {
        ...
        "source": "cloud-executive" if IS_CLOUD else "local-executive",
        "status": "pending_approval",
        "notes": notes,
    }

    # Write draft to Plans/cloud/
    PLANS_CLOUD_DIR.mkdir(parents=True, exist_ok=True)
    draft_file = PLANS_CLOUD_DIR / f"DRAFT_invoice_{draft_id}.md"
    draft_file.write_text(
        ...
        f"draft_only: true\namount: {total:.2f}\ncurrency: {currency}\n---\n\n"
        f"# Draft: Invoice for {partner_name}\n\n"
        f"## Invoice Details\n"
        f"- **Partner**: {partner_name}\n"
        f"- **Total**: {total:.2f} {currency}\n"
        f"- **Lines**:\n" +
        "".join(f"  - {l['description']}: {l.get('quantity',1)} x {l.get('price_unit',0):.2f}\n" for l in lines) +
        f"\n## Notes\n{notes or 'None'}\n\n"
        f"## Approval Required\n"
        f"- This draft was generated by the {'Cloud' if IS_CLOUD else 'Local'} Executive (draft-only)\n"
        f"- Human approval is required before posting to Odoo\n"
        f"- Approval file created in: `Pending_Approval/local/`\n",
        encoding="utf-8",
    )

    # Write approval request to Pending_Approval/local/
    PENDING_APPROVAL_DIR.mkdir(parents=True, exist_ok=True)
    approval_file = PENDING_APPROVAL_DIR / f"APPROVE_invoice_{draft_id}.md"
    approval_file.write_text(
        f"---\ntype: approval_request\naction: post_invoice\n"
        f"source: {draft_data['source']}\ncreated: {now.strftime('%Y-%m-%d %H:%M')}\n"
        f"correlation_id: {draft_id}\ndraft_file: {draft_file.name}\n---\n\n"
        f"# Approval Request: Invoice for {partner_name}\n\n"
        f"**Amount**: {total:.2f} {currency}\n"
        f"**Action**: Post invoice to Odoo\n\n"
        f"To approve: move this file to `Approved/`\n",
        encoding="utf-8",
    )

    # Write JSON sidecar for machine consumption
    json_file = PLANS_CLOUD_DIR / f"DRAFT_invoice_{draft_id}.json"
    json_file.write_text(json.dumps(draft_data, indent=2, default=str), encoding="utf-8")

    logger.info(f"[PLATINUM] Created invoice draft {draft_id} for {partner_name}: {total:.2f} {currency}")
...
def get_pending_drafts(draft_type=None):
    """Platinum: List pending invoice drafts awaiting approval.

    Args:
        draft_type: Optional filter (e.g. 'invoice_draft')
    """
    drafts = []
    if PLANS_CLOUD_DIR.exists():
        for json_file in sorted(PLANS_CLOUD_DIR.glob("DRAFT_invoice_*.json")):
            try:
                data = json.loads(json_file.read_text(encoding="utf-8"))
                if data.get("status") == "pending_approval":
                    if draft_type and data.get("type") != draft_type:
                        continue
                    data["file"] = json_file.name
                    drafts.append(data)
            except (json.JSONDecodeError, OSError) as e:
                logger.warning(f"Skipping unreadable draft {json_file.name}: {e}")

    logger.info(f"[PLATINUM] Found {len(drafts)} pending invoice drafts")
    return {"success": True, "drafts": drafts, "count": len(drafts)}


def post_invoice(draft_id):
    """Platinum: Post a previously drafted invoice to Odoo (LOCAL-ONLY).

    This reads the draft JSON, creates the invoice in Odoo, and posts it.
    Cloud agents are blocked from calling this tool.

    Args:
        draft_id: The draft_id from create_invoice_draft
    """
    _require_zone("local")
    ...
    if draft_data.get("status") != "pending_approval":
        return {"success": False, "error": f"Draft {draft_id} status is '{draft_data.get('status')}', expected 'pending_approval'"}

    partner_name = draft_data["partner_name"]
    lines = draft_data["lines"]
    currency = draft_data.get("currency", "USD")

    # Create and post invoice in Odoo
    result = create_invoice(partner_name, lines, currency)

    if not result.get("success"):
        return result

    # If not dry-run and invoice was created, post it
    if not DRY_RUN and result.get("invoice", {}).get("id"):
        client = OdooClient(ODOO_URL, ODOO_DB, ODOO_USERNAME, ODOO_PASSWORD)
        client.connect()
        client.execute("account.move", "action_post", [result["invoice"]["id"]])
        result["invoice"]["state"] = "posted"
    ...
    # Move approval file to Done if it exists in Approved/
    approved_file = APPROVED_DIR / f"APPROVE_invoice_{draft_id}.md"
    done_dir = VAULT_DIR_ROOT / "data" / "Done" / "local"
    done_dir.mkdir(parents=True, exist_ok=True)
    if approved_file.exists():
        approved_file.rename(done_dir / f"DONE_invoice_{draft_id}.md")

    # Write update for dashboard merge
    UPDATES_DIR.mkdir(parents=True, exist_ok=True)
    update_file = UPDATES_DIR / f"odoo_posted_{draft_id}.md"
    update_file.write_text(
        f"---\ntype: odoo_update\nsource: local-executive\n"
        f"timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M')}\n"
        f"summary: Posted invoice {draft_id} for {partner_name}\n---\n\n"
        f"# Odoo Update — Invoice Posted\n\n"
        f"- **Draft ID**: {draft_id}\n"
        f"- **Partner**: {partner_name}\n"
        f"- **Amount**: {draft_data.get('amount_total', 0):.2f} {currency}\n"
        f"- **Status**: Posted to Odoo\n",
        encoding="utf-8",
    )

    logger.info(f"[PLATINUM] Posted invoice {draft_id} for {partner_name}")
...
def approve_invoice(draft_id):
    """Platinum: Approve a draft invoice (LOCAL-ONLY).

    Moves the approval file from Pending_Approval to Approved.
    Does NOT post — call post_invoice separately after approval.
    ...
    # Move approval file
    pending_file = PENDING_APPROVAL_DIR / f"APPROVE_invoice_{draft_id}.md"
    if not pending_file.exists():
        return {"success": False, "error": f"Approval file for {draft_id} not found in Pending_Approval"}

    APPROVED_DIR.mkdir(parents=True, exist_ok=True)
    approved_path = APPROVED_DIR / f"APPROVE_invoice_{draft_id}.md"
    pending_file.rename(approved_path)

    # Update draft JSON status
    draft_data = json.loads(json_file.read_text(encoding="utf-8"))
    draft_data["status"] = "approved"
    draft_data["approved_at"] = datetime.now().isoformat()
    json_file.write_text(json.dumps(draft_data, indent=2, default=str), encoding="utf-8")

    logger.info(f"[PLATINUM] Approved invoice draft {draft_id}")

    return {
        "success": True,
        "draft_id": draft_id,
        "status": "approved",
        "next_step": "Call post_invoice to post to Odoo",
    }


# ── PLATINUM: Zone-Enforced Wrappers ────────────────────────────────────

def _cloud_guard_create_invoice(partner_name, lines, currency="USD"):
    """Cloud zone: redirect create_invoice to create_invoice_draft."""
    if IS_CLOUD:
        logger.info("[PLATINUM] Cloud zone: redirecting create_invoice → create_invoice_draft")
        return create_invoice_draft(partner_name, lines, currency)
    return create_invoice(partner_name, lines, currency)


def _cloud_guard_create_payment(partner_name, amount, payment_type="inbound", journal="Bank"):
    """Cloud zone: block payment creation entirely."""
    if IS_CLOUD:
        raise PermissionError(
            "Cloud agents cannot create payments. Delegate to local agent via "
            "Needs_Action/local/ with action: create_payment."
        )
    return create_payment(partner_name, amount, payment_type, journal)


def _cloud_guard_update_payment(payment_name, new_state):
    """Cloud zone: block payment state changes."""
    if IS_CLOUD:
        raise PermissionError(
            "Cloud agents cannot modify payment state. Delegate to local agent."
        )
    return update_payment(payment_name, new_state)


# ── MCP STDIO PROTOCOL ──────────────────────────────────────────────────
TOOLS = {
    # ── Gold Tier tools (zone-guarded for Platinum) ──────────────────────
    "create_invoice": {
        "description": "Create a customer invoice in Odoo (cloud zone auto-redirects to draft)",
        "parameters": {"partner_name": "string", "lines": "array", "currency": "string (optional)"},
        "handler": _cloud_guard_create_invoice
    },
    "get_invoices": {
    ...
    "update_payment": {
        "description": "Update the state of a payment record (blocked in cloud zone)",
        "parameters": {"payment_name": "string", "new_state": "string (draft|posted|cancelled)"},
        "handler": _cloud_guard_update_payment
    },
    "search_partners": {
    ...
    "create_invoice_draft": {
        "description": "Platinum: Create a draft invoice file (no Odoo write) for approval workflow",
        "parameters": {"partner_name": "string", "lines": "array", "currency": "string (optional)", "notes": "string (optional)"},
        "handler": create_invoice_draft
    },
    "get_pending_drafts": {
        "description": "Platinum: List pending invoice drafts awaiting approval",
        "parameters": {"draft_type": "string (optional)"},
        "handler": get_pending_drafts
    },
    "approve_invoice": {
        "description": "Platinum: Approve a draft invoice — moves to Approved/ (local-only)",
        "parameters": {"draft_id": "string"},
        "handler": approve_invoice
    },
    "post_invoice": {
        "description": "Platinum: Post an approved invoice to Odoo (local-only, after approval)",
        "parameters": {"draft_id": "string"},
...
def handle_request(request):
    ...
    if method == "initialize":
        return {"jsonrpc": "2.0", "id": req_id, "result": {
            "name": "odoo-mcp", "version": "2.0.0-platinum",
...
def run_stdio():
    ...
    logger.info(f"Odoo MCP server starting (DRY_RUN={DRY_RUN}, ZONE={VAULT_ENVIRONMENT})")
...
def run_test():
    ...
    global DRY_RUN
    DRY_RUN = True
    print(f"=== Odoo MCP Self-Test (DRY RUN, ZONE={VAULT_ENVIRONMENT}) ===\n")
    ...
    print("\n3. create_invoice (zone-guarded):")
    print(json.dumps(_cloud_guard_create_invoice("Test Client", [
    ...
    print("\n5. create_payment (zone-guarded):")
    try:
        print(json.dumps(_cloud_guard_create_payment("Test Client", 1500.00), indent=2))
    ...
    print("\n8. update_payment (zone-guarded):")
    try:
        print(json.dumps(_cloud_guard_update_payment("PAY/2026/0001", "cancelled"), indent=2))
    except PermissionError as e:
        print(f"  [BLOCKED] {e}")
    ...
    print("10. create_invoice_draft():")
    draft_result = create_invoice_draft("Platinum Test Corp", [
        {"description": "Cloud Hosting Q2", "quantity": 1, "price_unit": 2500},
        {"description": "Support Package", "quantity": 3, "price_unit": 500},
    ], notes="Auto-generated test draft")
    print(json.dumps(draft_result, indent=2))

    print("\n11. get_pending_drafts():")
    print(json.dumps(get_pending_drafts(), indent=2))

    if not IS_CLOUD:
        draft_id = draft_result.get("draft_id", "")
        print(f"\n12. approve_invoice('{draft_id}'):")
        print(json.dumps(approve_invoice(draft_id), indent=2))

        print(f"\n13. post_invoice('{draft_id}'):")
        print(json.dumps(post_invoice(draft_id), indent=2))
    else:
        print("\n12-13. [SKIPPED] approve_invoice/post_invoice blocked in cloud zone")

    print(f"\n=== All tests passed (zone: {VAULT_ENVIRONMENT}) ===")

#!/usr/bin/env python3
"""
Instagram Platinum Skill - Platinum Tier

Instagram integrations with Platinum zones: Cloud drafts, local exec.
Extends Gold IG Integrator with work-zone separation.
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - InstagramPlatinum - %(levelname)s - %(message)s",
    handlers=[
        logging.FileHandler(LOGS_DIR / "instagram_platinum.log"),
        logging.StreamHandler(),
    ],
)
logger = logging.getLogger("InstagramPlatinum")
...
def invoke_mcp(server_type: str, params: dict) -> dict:
    """
    Generic MCP invocation helper for Instagram integrations.

    Args:
        server_type: Type of MCP server (e.g., "social-ig")
    ...
    """
    try:
        if server_type == "social-ig":
            # Call the social-mcp-ig server (only available on local with secrets)
            mcp_script = VAULT_DIR / "mcp-servers" / "social-mcp-ig" / "social-mcp-ig.js"
...
def log_action(action_type: str, details: dict, result: str = "success"):
    ...
    try:
        ...
        correlation_id = f"ig_plat_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        audit_log_action(
            action_type=action_type,
            actor="instagram_platinum",
            target="social-mcp-ig" if VAULT_ENVIRONMENT == "local" else "cloud_processor",
            parameters=details,
            result=result,
        ...
        try:
            ...
            if tone_match:
                ...
                if social_match:
                    ...
                    return "Visual-focused, engaging tone. Use appropriate hashtags."
        except Exception as e:
...
def run_git_sync():
    ...
    try:
        ...
        # Commit changes
        commit_msg = f"Plat-IG sync {datetime.now().isoformat()}"
...
# ── MAIN INSTAGRAM PLATINUM SKILL ─────────────────────────────────────────
class InstagramPlatinum:
    """Main Instagram Platinum skill class with zone-aware operations."""

    def __init__(self):
        self.handbook_tone = read_company_handbook_tone()
        self.a2a_node = None

        logger.info(f"Instagram Platinum initialized for {VAULT_ENVIRONMENT} environment")
    ...
    def cloud_draft_post(self, message: str = None, plan_content: str = None) -> dict:
        """
        Cloud zone: Draft Instagram posts following Platinum spec.
        ...
        logger.info("Starting Cloud Instagram draft process")

        try:
            # Step 1: Create draft in /Plans/cloud/IG_DRAFT_{id}.md
            correlation_id = f"ig_draft_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

            if plan_content:
                # Use provided plan content
                draft_file = PLANS_CLOUD_DIR / f"IG_DRAFT_{correlation_id}.md"
...
**Hashtags:** #AIEmployee #VisualContent #SocialMedia
...
"""
            ...
            logger.info(f"Instagram draft created: {draft_file.name}")

            # Step 2: Write approval request to /Pending_Approval/local/IG_{id}.md
            approval_file = PENDING_APPROVAL_LOCAL / f"IG_{correlation_id}.md"
            approval_content = f"""---
...
Please review and approve this Instagram post. After approval, the local agent will execute the post.
"""
            ...
            update_file = UPDATES_DIR / f"cloud_ig_draft_{correlation_id}.md"
            update_content = f"""---
type: cloud_update
source: instagram_platinum
timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M')}
summary: Instagram draft created, approval pending
correlation_id: {correlation_id}
---

# Cloud Update: Instagram Draft Created
...
"""
            ...
            # Step 4: Run sync to push to local
            sync_success = run_git_sync()
            if not sync_success:
                logger.warning("Git sync failed after draft creation")

            # Step 5: Send A2A notification if available
            send_a2a_notification("draft_ready", {
                "draft_id": correlation_id,
                "draft_type": "instagram_post",
                "summary": f"Instagram draft ready for approval: {message[:50]}...",
                "created": datetime.now().isoformat()
            })
            ...
            result = {
                ...
                "approval_file": f"IG_{correlation_id}.md",
                "message": f"Instagram draft created and approval requested: {message[:50]}..."
            }

            # Log the success
            log_action("ig_draft_created", {
                "draft_file": str(draft_file.name),
                "approval_file": f"IG_{correlation_id}.md",
            ...
            logger.error(f"Error in cloud Instagram draft process: {e}")
            log_action("ig_draft_created", {
                "error": str(e),
            ...
            return {
                "success": False,
                "error": str(e),
                "message": f"Failed to create Instagram draft: {e}"
            }

    def local_process_approved_post(self, approved_file_path: Path) -> dict:
        """
        Local zone: Process approved Instagram posts and execute via MCP.
        ...
        logger.info(f"Processing approved Instagram post: {approved_file_path.name}")

        try:
            # Use claim-by-move to prevent double-processing
            ...
            match = re.search(r'IG_([a-zA-Z0-9_]+)', approved_file_path.name)
            if not match:
            ...
            for line in lines:
                if line.strip().startswith("**Draft Message**") or "**Message Preview**" in line:
            ...
            if not message:
                ...
                for line in lines:
                    if "**Draft Message**" in line or "**Message Preview:**" in line:
                        message = line.split(":", 1)[1].strip()
                        break
                if not message:
                    for line in lines:
                        if "Message Preview" in line:
                            message = line.split("Message Preview:")[-1].strip()
                            break

            if not message:
                raise ValueError("Could not find message to post")

            # Step 3: Execute via social-mcp-ig with local secrets
            mcp_params = {
                "action": "post",
                "message": message,
                "account_id": os.environ.get("IG_ACCOUNT_ID", "default_account"),  # Use local secrets
                "correlation_id": correlation_id
            }

            logger.info(f"Invoking MCP with params: {mcp_params}")
            mcp_result = invoke_mcp("social-ig", mcp_params)

            if mcp_result["success"]:
                logger.info(f"Instagram post successful: {correlation_id}")

                # Create execution update for dashboard
                exec_update_file = UPDATES_DIR / f"local_ig_exec_{correlation_id}.md"
                exec_update_content = f"""---
type: execution_result
source: instagram_platinum
timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M')}
summary: Instagram post executed successfully
correlation_id: {correlation_id}
---

# Local Execution: Instagram Post Complete

- **Post ID:** {correlation_id}
- **Message Posted:** {message[:100]}...
- **Status:** Success
...
"""
                ...
                log_action("instagram_post_success", {
                    "correlation_id": correlation_id,
                ...
                return {
                    ...
                    "status": "posted",
                    "correlation_id": correlation_id,
                    "message": f"Instagram post successful: {message[:50]}..."
                }
            else:
                ...
                error_update_file = UPDATES_DIR / f"local_ig_error_{correlation_id}.md"
                error_update_content = f"""---
type: execution_error
source: instagram_platinum
timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M')}
summary: Instagram post failed
correlation_id: {correlation_id}
---

# Local Execution: Instagram Post Failed

- **Post ID:** {correlation_id}
- **Message Attempted:** {message[:100]}...
- **Status:** Failed
- **Error:** {error_msg}
- **Executed:** {datetime.now().isoformat()}
"""
                ...
                log_action("instagram_post_failed", {
                    "correlation_id": correlation_id,
                    "message_preview": message[:100],
                    "error": error_msg
                }, "failed")
    ...
    def cloud_generate_summary_draft(self, engagement_data: dict = None) -> Path:
        """
        Cloud zone: Generate draft engagement summary.
        ...
        logger.info("Generating Cloud Instagram engagement summary draft")
        ...
        summary_file = BRIEFINGS_DIR / f"SUMMARY_IG_DRAFT_{date_str}.md"
...
# Instagram Engagement Summary Draft - {date_str}
...
"""
        ...
        logger.info(f"Cloud Instagram summary draft created: {summary_file.name}")
    ...
    def local_finalize_summary(self, draft_summaries: list) -> Path:
        """
        Local zone: Finalize summary from cloud draft and local data.
        ...
        logger.info(f"Finalizing Instagram engagement summary from {len(draft_summaries)} draft(s)")
        ...
        date_str = datetime.now().strftime('%Y%m')
        final_summary_file = BRIEFINGS_DIR / f"SUMMARY_IG_{date_str}.md"
...
# Instagram Engagement Summary - {date_str}
...
## Summary
This final summary incorporates data from both cloud and local Instagram metrics.

**Compiled by:** Instagram Platinum Local
**Finalized:** {datetime.now().isoformat()}
"""

        final_summary_file.write_text(final_summary_content, encoding="utf-8")
        logger.info(f"Final Instagram summary created: {final_summary_file.name}")
    ...
    def process_needs_action(self):
        """
        Process any Instagram-related items in the appropriate Needs_Action directory.
        Cloud processes cloud items, local processes local items.
        """
        logger.info(f"Processing Instagram needs_action items for {VAULT_ENVIRONMENT} zone")

        if VAULT_ENVIRONMENT == "cloud":
            # Cloud only processes cloud-specific needs (e.g., triage from skill-cloud-triage)
            needs_dir = NEEDS_ACTION_CLOUD
            action_pattern = r'.*_ig_.*\.md$'  # Files with ig in the name
        else:
        ...
        for item_file in needs_dir.glob(action_pattern):
            try:
                if claim_file(item_file, VAULT_ENVIRONMENT):
                    ...
                    items_processed += 1
                    logger.info(f"Processed Instagram needs_action item: {item_file.name}")
                else:
                    logger.info(f"Skipped already-claimed item: {item_file.name}")
            except Exception as e:
                logger.error(f"Error processing Instagram needs_action item {item_file.name}: {e}")

        logger.info(f"Processed {items_processed} Instagram needs_action items")

# ── MAIN EXECUTION ────────────────────────────────────────────────────────
def main():
    """Main function to demonstrate the Instagram Platinum skill."""
    import argparse

    parser = argparse.ArgumentParser(description="Instagram Platinum Skill")
    parser.add_argument("--draft", help="Create an Instagram post draft (cloud only)")
    parser.add_argument("--process-approved", help="Process an approved Instagram post file (local only)")
    ...
    if args.draft:
        ...
        print(f"Running Instagram Platinum demo for {VAULT_ENVIRONMENT} environment...")

        if VAULT_ENVIRONMENT == "cloud":
            print(f"\n1. Creating Instagram draft: Boost your business with visual content!")
            result = integrator.cloud_draft_post(message="Boost your business with visual content! Check out our latest designs! #AIEmployee #VisualContent")
            ...
            print(f"\n1. Processing Instagram posts in queue...")
            # Process any approved Instagram posts
            integrator.process_needs_action()

        print(f"\nPlatinum demo completed for {VAULT_ENVIRONMENT} zone!")

    else:
        print("Instagram Platinum Skill")

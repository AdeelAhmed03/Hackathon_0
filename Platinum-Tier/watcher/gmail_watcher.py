# Platinum-tier Gmail Watcher for Digital FTE
# Polls for unread/important emails → creates .md in data/Needs_Action/cloud/
# Cloud zone: triage emails into cloud work queue for draft generation
...
# Platinum: Route to cloud zone — email triage is a cloud task
...
class GmailWatcher:
    ...
    def run(self):
        ...
        # Gold Tier: log watcher startup to audit log
        if HAS_AUDIT_LOGGER:
            log_action(
                action_type="watcher.start",
                actor="gmail_watcher",
                target="gmail_api",
                parameters={"poll_interval": CHECK_INTERVAL_SECONDS},
                result="success",
                severity="INFO",
            )
    ...
    def check_for_new_emails(self):
        ...
        if not messages:
            ...
            # Gold Tier: log polling result to audit log
            if HAS_AUDIT_LOGGER:
                log_action(
                    action_type="email.check",
                    actor="gmail_watcher",
                    target="gmail_api",
                    parameters={"query": QUERY, "found_count": 0},
                    result="success",
                    severity="DEBUG",
        ...
        if HAS_AUDIT_LOGGER:
            log_action(
                ...
                actor="gmail_watcher",
                target="gmail_api",
                parameters={"query": QUERY, "found_count": len(messages)},
                result="success",
                severity="INFO",
                duration_ms=int((time.time() - start_time) * 1000),
            )
...
"""
        ...
        # Gold Tier: log email creation to audit log
        if HAS_AUDIT_LOGGER:
            log_action(
                action_type="email.processed",
                actor="gmail_watcher",
                target=f"EMAIL_{msg_id}",
                parameters={"from": from_, "subject": subject, "snippet_len": len(snippet)},
                result="success",
                severity="INFO",
            )

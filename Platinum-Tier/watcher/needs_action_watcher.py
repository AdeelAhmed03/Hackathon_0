#!/usr/bin/env python3
"""
Enhanced Needs Action Watcher (Platinum Tier)

Watches data/Needs_Action/cloud/ and data/Needs_Action/local/ for new .md files
based on VAULT_ENVIRONMENT zone. Automatically runs the Ralph Wiggum agent.
Also supports data/Inbox/ for future drop-folder style input.

Platinum zone awareness:
  cloud  → watches data/Needs_Action/cloud/ only
  local  → watches data/Needs_Action/local/ only
  both   → watches both cloud/ and local/ subdirectories (dev mode)

Gold Tier: Error recovery with quarantine, references agent-autonomous-employee.md.
"""
import time
import logging
import subprocess
import sys
import os
import argparse
import threading
from pathlib import Path
from watchdog.observers import Observer
from watchdog.events import FileSystemEventHandler

try:
    from plyer import notification
    PLYER_AVAILABLE = True
except ImportError:
    PLYER_AVAILABLE = False
    print("Warning: plyer not installed. Desktop notifications will be disabled.")

# ── CONFIG ────────────────────────────────────────────────────────────────
VAULT_DIR = Path(__file__).parent.parent.resolve()    # project root (one level up from watcher/)
LOGS_DIR = VAULT_DIR / "data" / "Logs"
LOG_FILE = LOGS_DIR / "watcher.log"

# Gold Tier: Error recovery configuration
MAX_RETRY_ATTEMPTS = int(os.environ.get("MAX_RETRY_ATTEMPTS", "3"))
QUARANTINE_ON_FAILURE = os.environ.get("QUARANTINE_ON_FAILURE", "true").lower() == "true"
QUARANTINE_DIR = VAULT_DIR / "data" / "Quarantine"

# Platinum: Zone-aware directories
VAULT_ENVIRONMENT = os.environ.get("VAULT_ENVIRONMENT", "both")
NEEDS_ACTION_CLOUD = VAULT_DIR / "data" / "Needs_Action" / "cloud"
NEEDS_ACTION_LOCAL = VAULT_DIR / "data" / "Needs_Action" / "local"

# ── AUDIT LOGGER INTEGRATION (Gold Tier) ─────────────────────────────────
try:
    from audit_logger import log_action, log_error as _audit_log_error
    HAS_AUDIT_LOGGER = True
except ImportError:
    HAS_AUDIT_LOGGER = False

# Default Ralph command - Gold Tier references agent-autonomous-employee.md
DEFAULT_RALPH_COMMAND = os.environ.get(
    "RALPH_COMMAND",
    'claude /ralph-loop "Process all files in data/Needs_Action/ using Autonomous Employee (agents/agent-autonomous-employee.md). '
    'Also: check data/Approved/ via skill-hitl-watcher, run skill-scheduler time checks, '
    'check data/Accounting/ for Odoo tasks, and run error recovery on any failures." '
    '--max-iterations 20 --completion-promise "TASK_COMPLETE"'
)


# ── LOGGING SETUP ─────────────────────────────────────────────────────────
def setup_logging():
    LOGS_DIR.mkdir(parents=True, exist_ok=True)
    
    logging.basicConfig(
        level=logging.INFO,
        format='%(asctime)s - %(levelname)s - %(message)s',
        handlers=[
            logging.FileHandler(LOG_FILE),
            logging.StreamHandler(sys.stdout)
        ]
    )
    return logging.getLogger("NeedsActionWatcher")


# ── UTILITY FUNCTIONS ─────────────────────────────────────────────────────
def check_claude_available():
    """Check if claude command is available in PATH."""
    try:
        result = subprocess.run(['which', 'claude'], 
                              capture_output=True, text=True, timeout=10)
        return result.returncode == 0
    except:
        try:
            # Alternative check for Windows
            result = subprocess.run(['where', 'claude'], 
                                  capture_output=True, text=True, timeout=10)
            return result.returncode == 0
        except:
            return False


def parse_ralph_command(command_str):
    """Parse the Ralph command string into a list for subprocess."""
    # Simple parsing that handles quoted arguments
    import shlex
    try:
        return shlex.split(command_str)
    except ValueError as e:
        logger.error(f"Error parsing command '{command_str}': {e}")
        return []


def send_notification(title, message):
    """Send a desktop notification if plyer is available."""
    if PLYER_AVAILABLE:
        try:
            notification.notify(
                title=title,
                message=message,
                app_name="AI Employee Vault",
                timeout=5
            )
        except Exception as e:
            logger.warning(f"Failed to send notification: {e}")
    else:
        logger.debug(f"Notification: {title} - {message}")


def cleanup_empty_files(directory):
    """Remove empty .md files from the specified directory."""
    if not directory.exists():
        return
        
    removed_count = 0
    for file_path in directory.glob("*.md"):
        if file_path.stat().st_size == 0:  # File is empty
            try:
                file_path.unlink()
                logger.info(f"Removed empty file: {file_path.name}")
                removed_count += 1
            except Exception as e:
                logger.error(f"Failed to remove empty file {file_path.name}: {e}")
    
    if removed_count > 0:
        logger.info(f"Cleanup completed: removed {removed_count} empty files")


def health_check_loop():
    """Background thread that logs health check messages every 10 minutes."""
    while not stop_event.is_set():
        time.sleep(600)  # Sleep for 10 minutes
        if not stop_event.is_set():  # Double-check after sleep
            logger.info("Watcher alive - health check")


# ── EVENT HANDLER ─────────────────────────────────────────────────────────
class NeedsActionHandler(FileSystemEventHandler):
    def __init__(self, dry_run=False, ralph_command=None, needs_action_dir=None, inbox_dir=None, cleanup_enabled=False):
        self.dry_run = dry_run
        self.ralph_command = ralph_command or DEFAULT_RALPH_COMMAND
        self.needs_action_dir = needs_action_dir
        self.inbox_dir = inbox_dir
        self.cleanup_enabled = cleanup_enabled
        self.parsed_command = parse_ralph_command(self.ralph_command)

    def _matches_dir(self, file_path, target_dir):
        """Compare paths reliably on Windows (case-insensitive, resolved)."""
        if target_dir is None:
            return False
        return file_path.resolve().parent == target_dir.resolve()

    def on_created(self, event):
        if event.is_directory:
            return
        path = Path(event.src_path)
        if path.suffix.lower() == '.md':
            if self._matches_dir(path, self.needs_action_dir):
                logger.info(f"New task file detected in Needs_Action: {path.name}")
                self.trigger_agent(path, "Needs_Action")
            elif self._matches_dir(path, self.inbox_dir):
                logger.info(f"New file detected in Inbox: {path.name}")
                self.trigger_agent(path, "Inbox")

    def on_modified(self, event):
        if event.is_directory:
            return
        path = Path(event.src_path)
        if path.suffix.lower() == '.md':
            if self._matches_dir(path, self.needs_action_dir) or \
               self._matches_dir(path, self.inbox_dir):
                logger.info(f"Task file modified: {path.name}")
                self.trigger_agent(path, "Modified")

    def _quarantine_on_failure(self, file_path, reason):
        """Move a failed task file to quarantine (Gold Tier error recovery)."""
        if not QUARANTINE_ON_FAILURE:
            # Gold Tier: log that quarantine is disabled
            if HAS_AUDIT_LOGGER:
                log_action(
                    action_type="quarantine.skipped",
                    actor="needs_action_watcher",
                    target=file_path.name,
                    parameters={"reason": reason, "quarantine_disabled": True},
                    result="skipped",
                    severity="WARN",
                )
            return

        start_time = time.time()
        try:
            QUARANTINE_DIR.mkdir(parents=True, exist_ok=True)
            from datetime import datetime
            now = datetime.now()
            quarantine_name = f"QUARANTINE_{file_path.stem}_{now.strftime('%Y%m%d')}.md"
            dest = QUARANTINE_DIR / quarantine_name
            if file_path.exists():
                # Read original content and prepend quarantine metadata
                original = file_path.read_text(encoding="utf-8")
                quarantine_header = (
                    f"---\ntype: quarantined_task\n"
                    f"original_file: {file_path.name}\n"
                    f"quarantine_reason: \"{reason}\"\n"
                    f"quarantine_date: {now.isoformat()}\n"
                    f"recoverable: true\n---\n\n"
                )
                dest.write_text(quarantine_header + original, encoding="utf-8")
                file_path.unlink()
                logger.warning(f"Quarantined {file_path.name} → {quarantine_name}: {reason}")

                # Gold Tier: log successful quarantine to audit log
                if HAS_AUDIT_LOGGER:
                    log_action(
                        action_type="task.quarantined",
                        actor="needs_action_watcher",
                        target=file_path.name,
                        parameters={
                            "quarantine_name": quarantine_name,
                            "reason": reason,
                            "original_file": file_path.name
                        },
                        result="success",
                        severity="ERROR",
                        duration_ms=int((time.time() - start_time) * 1000),
                    )
            else:
                logger.info(f"File {file_path.name} no longer exists, skip quarantine")

                # Gold Tier: log that file no longer exists
                if HAS_AUDIT_LOGGER:
                    log_action(
                        action_type="quarantine.skipped",
                        actor="needs_action_watcher",
                        target=file_path.name,
                        parameters={"reason": reason, "file_missing": True},
                        result="skipped",
                        severity="WARN",
                        duration_ms=int((time.time() - start_time) * 1000),
                    )
        except Exception as qe:
            logger.error(f"Failed to quarantine {file_path.name}: {qe}")

            # Gold Tier: log quarantine failure to audit log
            if HAS_AUDIT_LOGGER:
                _audit_log_error(
                    action_type="quarantine.failed",
                    actor="needs_action_watcher",
                    target=file_path.name,
                    error=qe,
                    parameters={"reason": reason},
                    duration_ms=int((time.time() - start_time) * 1000),
                )

    def trigger_agent(self, file_path, source_dir):
        start_time = time.time()

        # Platinum: Zone-aware claim-by-move pattern
        # Determine zone from file location: cloud/ or local/
        file_parent = file_path.resolve().parent.name
        if file_parent == "cloud":
            in_progress_dir = VAULT_DIR / "data" / "In_Progress" / "cloud"
        elif file_parent == "local":
            in_progress_dir = VAULT_DIR / "data" / "In_Progress" / "local"
        else:
            # Fallback: legacy Needs_Action/ (no zone subdirectory)
            in_progress_dir = VAULT_DIR / "data" / "In_Progress" / "agent-autonomous-employee"
        in_progress_dir.mkdir(parents=True, exist_ok=True)

        # Try to claim the file by moving it to In_Progress
        claimed_file_path = in_progress_dir / file_path.name

        if self.dry_run:
            logger.info(f"[DRY RUN] Would claim: {file_path.name} → {claimed_file_path}")
        else:
            try:
                # Attempt to move the file to claim it
                file_path.rename(claimed_file_path)
                logger.info(f"Claimed file: {file_path.name} → {claimed_file_path}")
            except FileNotFoundError:
                # File already moved by another process, so we skip it
                logger.info(f"File {file_path.name} already claimed by another process, skipping")

                # Gold Tier: log skipped trigger to audit log
                if HAS_AUDIT_LOGGER:
                    log_action(
                        action_type="agent.skipped",
                        actor="needs_action_watcher",
                        target=file_path.name,
                        parameters={"reason": "file_already_claimed", "source_dir": source_dir},
                        result="skipped",
                        severity="INFO",
                        duration_ms=int((time.time() - start_time) * 1000),
                    )
                return
            except Exception as e:
                logger.error(f"Failed to claim file {file_path.name}: {e}")

                # Gold Tier: log claim failure to audit log
                if HAS_AUDIT_LOGGER:
                    _audit_log_error(
                        action_type="agent.claim_failed",
                        actor="needs_action_watcher",
                        target=file_path.name,
                        error=e,
                        parameters={"source_dir": source_dir},
                        duration_ms=int((time.time() - start_time) * 1000),
                    )
                return

        # Now update the Ralph command to focus on processing the claimed file in In_Progress
        # This is a simplified approach - in reality the agent would be instructed to focus on In_Progress
        ralph_command_with_claimed_focus = self.ralph_command.replace(
            "data/Needs_Action/",
            "data/In_Progress/agent-autonomous-employee/ data/Needs_Action/"
        )

        parsed_claimed_command = parse_ralph_command(ralph_command_with_claimed_focus)

        try:
            # Prevent multiple overlapping runs (simple lock)
            lock_file = VAULT_DIR / ".agent_running.lock"
            if lock_file.exists():
                logger.info("Agent already running → skipping")

                # Gold Tier: log skipped trigger to audit log
                if HAS_AUDIT_LOGGER:
                    log_action(
                        action_type="agent.skipped",
                        actor="needs_action_watcher",
                        target=file_path.name,
                        parameters={"reason": "agent_already_running", "source_dir": source_dir},
                        result="skipped",
                        severity="WARN",
                        duration_ms=int((time.time() - start_time) * 1000),
                    )
                return

            if self.dry_run:
                logger.info(f"[DRY RUN] Would process: {claimed_file_path.name} from {source_dir}")
                logger.info(f"[DRY RUN] Would run command: {' '.join(parsed_claimed_command)}")

                # Gold Tier: log dry-run to audit log
                if HAS_AUDIT_LOGGER:
                    log_action(
                        action_type="agent.trigger",
                        actor="needs_action_watcher",
                        target=claimed_file_path.name,
                        parameters={"source_dir": source_dir, "dry_run": True},
                        result="success",
                        severity="INFO",
                        duration_ms=int((time.time() - start_time) * 1000),
                    )
                return

            lock_file.touch()
            logger.info(f"Starting Ralph Wiggum agent loop for: {claimed_file_path.name} from {source_dir}")
            send_notification("Agent Started", f"Processing {claimed_file_path.name}")

            # Gold Tier: log agent start to audit log
            if HAS_AUDIT_LOGGER:
                log_action(
                    action_type="agent.trigger",
                    actor="needs_action_watcher",
                    target=claimed_file_path.name,
                    parameters={"source_dir": source_dir},
                    result="success",
                    severity="INFO",
                )

            # Run claude in a subprocess (non-blocking if you want, but for now we wait)
            result = subprocess.run(
                parsed_claimed_command,
                cwd=VAULT_DIR,               # important: run from project root
                capture_output=True,
                text=True,
                timeout=3600                 # safety timeout 1 hour
            )

            if result.returncode == 0:
                logger.info("Agent loop completed successfully")
                send_notification("Agent Completed", "Processing completed successfully")

                # Gold Tier: log successful completion to audit log
                if HAS_AUDIT_LOGGER:
                    log_action(
                        action_type="agent.completed",
                        actor="needs_action_watcher",
                        target=claimed_file_path.name,
                        parameters={"source_dir": source_dir},
                        result="success",
                        severity="INFO",
                        duration_ms=int((time.time() - start_time) * 1000),
                    )

                # Perform cleanup if enabled
                if self.cleanup_enabled:
                    logger.info("Performing cleanup of empty files...")
                    cleanup_empty_files(self.needs_action_dir)
                    cleanup_empty_files(in_progress_dir)
                    if self.inbox_dir:
                        cleanup_empty_files(self.inbox_dir)
            else:
                logger.error(f"Agent failed:\nSTDOUT: {result.stdout}\nSTDERR: {result.stderr}")
                send_notification("Agent Failed", "Processing failed - check logs")

                # Gold Tier: log failure to audit log
                if HAS_AUDIT_LOGGER:
                    _audit_log_error(
                        action_type="agent.failed",
                        actor="needs_action_watcher",
                        target=claimed_file_path.name,
                        error=Exception(f"Agent failed with return code {result.returncode}"),
                        parameters={
                            "source_dir": source_dir,
                            "return_code": result.returncode,
                            "stdout_len": len(result.stdout),
                            "stderr_len": len(result.stderr)
                        },
                        duration_ms=int((time.time() - start_time) * 1000),
                    )

        except subprocess.TimeoutExpired:
            logger.error("Agent loop timed out after 1h")
            send_notification("Agent Timeout", "Processing timed out after 1 hour")

            # Gold Tier: log timeout to audit log
            if HAS_AUDIT_LOGGER:
                _audit_log_error(
                    action_type="agent.timeout",
                    actor="needs_action_watcher",
                    target=claimed_file_path.name,
                    error=Exception("Agent loop exceeded 1 hour timeout"),
                    parameters={"source_dir": source_dir},
                    duration_ms=int((time.time() - start_time) * 1000),
                )

            self._quarantine_on_failure(claimed_file_path, "Timeout: agent loop exceeded 1 hour")
        except Exception as e:
            logger.exception(f"Error starting agent: {e}")
            send_notification("Agent Error", f"Error occurred: {str(e)[:50]}...")

            # Gold Tier: log error to audit log
            if HAS_AUDIT_LOGGER:
                _audit_log_error(
                    action_type="agent.error",
                    actor="needs_action_watcher",
                    target=claimed_file_path.name,
                    error=e,
                    parameters={"source_dir": source_dir},
                    duration_ms=int((time.time() - start_time) * 1000),
                )

            self._quarantine_on_failure(claimed_file_path, f"Exception: {str(e)}")
        finally:
            if lock_file.exists():
                lock_file.unlink()


# ── MAIN ──────────────────────────────────────────────────────────────────
def main():
    global logger, stop_event
    
    parser = argparse.ArgumentParser(
        description="Watch data/Needs_Action/{cloud,local}/ for new .md files and run Ralph Wiggum agent (Platinum)"
    )
    parser.add_argument("--dry-run", action="store_true",
                       help="Don't actually run the agent, just log what would happen")
    parser.add_argument("--command", type=str, default=DEFAULT_RALPH_COMMAND,
                       help="Custom command to run instead of default Ralph command")
    parser.add_argument("--dir", type=str, default=None,
                       help="Directory to watch (default: auto-detect from VAULT_ENVIRONMENT zone)")
    parser.add_argument("--zone", choices=["cloud", "local", "both"], default=VAULT_ENVIRONMENT,
                       help=f"Override zone (default: {VAULT_ENVIRONMENT} from VAULT_ENVIRONMENT)")
    parser.add_argument("--inbox-dir", type=str, default="data/Inbox",
                       help="Additional directory to watch for new files (default: data/Inbox, set to empty to disable)")
    parser.add_argument("--max-iterations", type=int, default=30,
                       help="Max iterations for the agent (passed to command if applicable)")
    parser.add_argument("--cleanup", action="store_true",
                       help="Enable cleanup of empty .md files after successful agent run")
    parser.add_argument("--check-claude", action="store_true",
                       help="Check if claude is available and exit")
    
    args = parser.parse_args()
    
    logger = setup_logging()
    
    if args.check_claude:
        if check_claude_available():
            print("✓ claude command is available in PATH")
            return 0
        else:
            print("✗ claude command is NOT available in PATH")
            return 1
    
    if not args.dry_run and not check_claude_available():
        logger.error("claude command not found in PATH. Please install claude or check your PATH.")
        sys.exit(1)
    
    # Platinum: Zone-aware directory setup
    zone = args.zone
    if args.dir:
        # Explicit directory override
        watch_dirs = [VAULT_DIR / args.dir]
    elif zone == "cloud":
        watch_dirs = [NEEDS_ACTION_CLOUD]
    elif zone == "local":
        watch_dirs = [NEEDS_ACTION_LOCAL]
    else:  # "both"
        watch_dirs = [NEEDS_ACTION_CLOUD, NEEDS_ACTION_LOCAL]

    inbox_dir = VAULT_DIR / args.inbox_dir if args.inbox_dir.strip() != "" else None

    # Create directories if they don't exist
    for d in watch_dirs:
        d.mkdir(parents=True, exist_ok=True)
        logger.info(f"Created/verified watch directory: {d}")

    if inbox_dir:
        inbox_dir.mkdir(parents=True, exist_ok=True)
        logger.info(f"Created/verified directory: {inbox_dir}")

    # Update the command with the max-iterations value if it contains the placeholder
    ralph_command = args.command
    if "--max-iterations" in ralph_command:
        # Replace the existing max-iterations value
        import re
        ralph_command = re.sub(r"--max-iterations\s+\d+", f"--max-iterations {args.max_iterations}", ralph_command)
    else:
        # Add max-iterations to the command
        ralph_command += f" --max-iterations {args.max_iterations}"

    # Use first watch_dir as primary for the handler
    event_handler = NeedsActionHandler(
        dry_run=args.dry_run,
        ralph_command=ralph_command,
        needs_action_dir=watch_dirs[0],
        inbox_dir=inbox_dir,
        cleanup_enabled=args.cleanup
    )

    observer = Observer()
    # Watch all zone-appropriate directories
    for d in watch_dirs:
        observer.schedule(event_handler, str(d), recursive=False)
        logger.info(f"Watching: {d}")

    # Also watch inbox directory if it exists and is different
    if inbox_dir and inbox_dir not in watch_dirs:
        observer.schedule(event_handler, str(inbox_dir), recursive=False)

    # Start health check thread
    stop_event = threading.Event()
    health_thread = threading.Thread(target=health_check_loop, daemon=True)
    health_thread.start()

    observer.start()

    mode = "DRY RUN" if args.dry_run else "ACTIVE"
    logger.info(f"Started {mode} watching (zone={zone}): {[str(d) for d in watch_dirs]}")
    if inbox_dir:
        logger.info(f"Also watching: {inbox_dir}")
    logger.info(f"Using command: {ralph_command}")
    if args.cleanup:
        logger.info("Cleanup enabled: will remove empty .md files after successful runs")

    try:
        while True:
            time.sleep(1)
    except KeyboardInterrupt:
        stop_event.set()  # Signal health thread to stop
        observer.stop()
        logger.info("Watcher stopped by user")
    observer.join()


if __name__ == "__main__":
    main()
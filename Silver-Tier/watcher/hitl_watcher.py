...
DEFAULT_HITL_COMMAND = os.environ.get(
    ...
    '--max-iterations 10 --completion-promise "TASK_COMPLETE"'
)
...
class ApprovedFileHandler(FileSystemEventHandler):
    ...
    def _process_file(self, file_path):
        ...
        if self.dry_run:
            logger.info(f"[DRY RUN] Would process approved file: {file_path.name}")
            logger.info(f"[DRY RUN] Would run command: {' '.join(self.parsed_command)}")
            return

        try:
            LOCK_FILE.touch()
            logger.info(f"Starting HITL agent for: {file_path.name}")
            send_notification("HITL Agent Started", f"Processing approved: {file_path.name}")
            ...
            if result.returncode == 0:
                logger.info(f"HITL agent completed successfully for: {file_path.name}")
                send_notification(
                    "HITL Completed", f"Processed: {file_path.name}"
                )
            else:
                logger.error(
                    f"HITL agent failed for {file_path.name}:\n"
                    f"STDOUT: {result.stdout[:500]}\n"
                    f"STDERR: {result.stderr[:500]}"
                )
                send_notification("HITL Failed", f"Failed: {file_path.name} - check logs")

        except subprocess.TimeoutExpired:
            logger.error(f"HITL agent timed out for: {file_path.name}")
            send_notification("HITL Timeout", f"Timed out: {file_path.name}")
        except Exception as e:
            logger.exception(f"Error in HITL agent for {file_path.name}: {e}")
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Max iterations for the HITL agent (default: 10)",
    )
    parser.add_argument(
        "--check-claude",
        action="store_true",
